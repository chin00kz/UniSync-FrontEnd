    ...
    <div className="space-y-8">
          ...
          <div key={item.id} className="premium-card hover:translate-y-[-4px] transition-all border-slate-200/60 p-6 flex flex-col gap-4 group text-left">
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-50 mt-2">
              <div className="flex flex-col">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-tighter">File Type</span>
      ...
      {mockOrganizedContent.length > 0 && (
        <div className="bg-slate-50 rounded-2xl p-8 border border-dashed border-slate-200 text-center">
          <p className="text-slate-500 font-medium italic">"The beautiful thing about learning is that no one can take it away from you." — B.B. King</p>
        </div>
      )}
    </div>
  );
